...
def test_codex_healthcheck_reports_success_when_mcp_and_gpt_are_healthy():
    def runner(args: list[str]) -> FakeCompletedProcess:
        if args[:3] == ["claude", "mcp", "list"]:
            return FakeCompletedProcess(returncode=0, stdout="codex: codex mcp-server - ✓ Connected\n")
...
def test_codex_healthcheck_reports_missing_claude_binary():
    def runner(args: list[str]) -> FakeCompletedProcess:
        if args[:3] == ["claude", "mcp", "list"]:
    ...
    assert report["stage"] == "claude-missing"


def test_codex_healthcheck_treats_shell_command_not_found_as_missing_claude():
    def runner(args: list[str]) -> FakeCompletedProcess:
        if args[:3] == ["claude", "mcp", "list"]:
...
def test_codex_healthcheck_reports_disconnected_mcp():
    def runner(args: list[str]) -> FakeCompletedProcess:
        if args[:3] == ["claude", "mcp", "list"]:
            return FakeCompletedProcess(returncode=0, stdout="codex: codex mcp-server - x Disconnected\n")
        raise AssertionError(f"Unexpected command: {args}")

    report = run_codex_healthcheck(command_runner=runner)

    assert report["ok"] is False
    assert report["stage"] == "mcp-disconnected"
...
def test_default_command_runner_uses_utf8_safe_decoding(monkeypatch):
    ...
    _default_command_runner(["claude", "mcp", "list"])
