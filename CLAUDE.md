...
Claude is the primary operator for this repository. Keep the workflow grounded in the canonical project files and the repo-local CLI rather than relying on conversational memory.
...
- Claude remains the primary operator and sole owner of canonical project state.
- When Codex MCP is configured in the user's local environment, Claude may request GPT-5.4 advisory review through Codex at selected checkpoints.
...
- Do not ask the user to memorize or manually invoke routine scripts when Claude can safely run them instead.
...
Claude should normally execute these directly when the user's intent is clear:
...
- `workspace/dashboard-data.json` and `workspace/dashboard.html` are derived views, never source of truth.
...
- When Claude needs operator input, present 2-4 concrete options with one recommended default.
- Each option should state what Claude will do next and the main tradeoff.
