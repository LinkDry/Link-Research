...
| `reflect` | summarize session learnings and update reusable memory | `projects/<slug>/STATE.md`, `projects/<slug>/experiment-memory.md`, `projects/<slug>/project-brief.md`, `projects/<slug>/review-state.json`, `memory/lessons-learned.md`, optional archive refs | `memory/lessons-learned.md`, `projects/<slug>/STATE.md` summary fields only | `results.tsv`, `review-state.json`, `project-brief.md`, `memory/failure-library.md`, judge-owned logs, archive records, anchor files, dashboard projection | catch-all mutation, frontmatter drift, writing files it only meant to read | V2 should be scoped to user-facing session synthesis, memory promotion, and compact state refresh only; session summaries remain output, not a new owned file |
| `overnight` | orchestrate resumable Phase 1 or Phase 2 runs | `projects/<slug>/STATE.md`, `projects/<slug>/project-brief.md`, `projects/<slug>/experiment-memory.md`, `projects/<slug>/review-state.json` | `projects/<slug>/review-state.json`, run-lifecycle fields in `projects/<slug>/STATE.md` | `experiment-memory.md`, `results.tsv`, anchors, `decision-tree.md`, archive records, memory files, dashboard projection as state | fixed mega-plan drift, missing human-gated pause semantics, broken pause/resume ownership | V2 should use `review-state.json` as the only run controller, use a fixed bootstrap segment only until an active line exists, then route from canonical state one approved step at a time; Phase 2 delegates to `phase2-publish` |
...
- Must keep `review-state.json`, `project-brief.md`, and `memory/failure-library.md` outside its write surface.

### `overnight`

- Must orchestrate only approved sequences.
- Must route from canonical state one approved step at a time once an active line exists.
...
- Must delegate publication semantics to the approved Phase 2 workflow instead of embedding a second copy.

## Open Questions To Resolve During Skill Authoring

1. When branch variants exist, should the active `experiment-plan.md` be replaced in place or preserved and linked through explicit archival/version refs?
