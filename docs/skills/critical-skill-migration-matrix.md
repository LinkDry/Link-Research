...
4. If a decision is human-gated, the skill must express it through `review-state.json` or a clearly referenced decision block, not hidden prose.
...
|------|------------------|-----------------|------------------|---------------------|-----------------------|--------------------|
| `anchor-wrapper` | lock experiment anchor before execution | `projects/<slug>/project-brief.md`, `projects/<slug>/STATE.md`, `projects/<slug>/experiment-memory.md`, experiment-plan input | `projects/<slug>/plans/<idea>/anchor.md`, `projects/<slug>/experiment-memory.md`, `projects/<slug>/STATE.md` | `results.tsv`, `review-state.json`, dashboard projection | invented fields, index side effects, mutable anchor behavior | V2 should write the anchor record plus only the minimal project/experiment references needed to bind it |
| `drift-detector` | compare active experiment against locked anchor | `projects/<slug>/plans/<idea>/anchor.md`, `projects/<slug>/experiment-memory.md`, `projects/<slug>/results.tsv`, workspace artifacts as read-only evidence | `projects/<slug>/experiment-memory.md`, `projects/<slug>/STATE.md` | `anchor.md`, `results.tsv`, global memory | frontmatter/body drift, unclear workspace ownership, ambiguous decision writeback | V2 should treat workspace as evidence input only and store drift outcome in experiment/project state |
| `judge` | issue Phase 1 verdict and next-step recommendation | `projects/<slug>/plans/<idea>/anchor.md`, `projects/<slug>/experiment-memory.md`, `projects/<slug>/results.tsv`, `projects/<slug>/decision-tree.md`, `memory/lessons-learned.md` | `projects/<slug>/experiment-memory.md`, `projects/<slug>/STATE.md`, `projects/<slug>/decision-tree.md` | `anchor.md`, `results.tsv`, dashboard projection | multiple archive policies, over-broad writes, hidden coupling to verdict log | V2 should keep verdict state canonical in experiment/project views and treat cross-model review as advisory evidence |
| `archive` | close an experiment line and extract reusable lessons | `projects/<slug>/STATE.md`, `projects/<slug>/experiment-memory.md`, `projects/<slug>/results.tsv`, `projects/<slug>/decision-tree.md`, `projects/<slug>/project-brief.md`, relevant workspace artifacts | `projects/<slug>/experiment-memory.md`, `projects/<slug>/STATE.md`, `projects/<slug>/archive/*.md`, `memory/lessons-learned.md`, `memory/failure-library.md` | `results.tsv`, anchor files, dashboard projection | unsafe deletion, mixed local/global archive boundaries, template mismatch | V2 should be move-or-snapshot oriented, preserve ledgers, and promote reusable failure patterns globally |
...
- Must treat `results.tsv` plus experiment history as the evidence basis.
...
1. Where should branch-creation decisions be represented when `judge` recommends a split: directly in `decision-tree.md`, in `review-state.json`, or both?
2. Should `reflect` own capability-gap promotion into `memory/failure-library.md`, or should that remain `archive`-only?
3. How much of the Phase 2 flow should remain inside `overnight` versus delegated wholly to a future V2 `phase2-publish` skill?
4. What minimal experiment-plan input format does V2 require before `anchor-wrapper` can safely lock an anchor?
