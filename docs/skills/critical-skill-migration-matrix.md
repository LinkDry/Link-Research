...
1. Should `reflect` own capability-gap promotion into `memory/failure-library.md`, or should that remain `archive`-only?
2. How much of the Phase 2 flow should remain inside `overnight` versus delegated wholly to a future V2 `phase2-publish` skill?
3. When branch variants exist, should the active `experiment-plan.md` be replaced in place or preserved and linked through explicit archival/version refs?
