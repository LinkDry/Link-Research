...
If the same warning signal or lesson pattern recurs, `reflect` should add a queue item to `Promotion Queue` rather than directly modifying global governance docs or the failure library.

This keeps pattern promotion explicit and reviewable.

## Housekeeping Boundary

`reflect` does not trigger `/garden` or any equivalent hidden maintenance flow.
